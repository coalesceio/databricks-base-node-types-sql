@id("d2ceba50-ea2e-4bea-a943-6c5d3cddc609")
@nodeType("708")
@writeMode("append")
@description("Nation stage table")
@tests("SELECT 1 WHERE 1<>0")
@preSQL("SELECT 1")
@postSQL("SELECT 1")
SELECT
     `n_nationkey` AS `n_nationkey` @notNull @not_null @uniqueness @min_value("0") @max_value("24") @inHash("GH_NATION", "1"),
     ...
     CAST({{get_hash(GH_NATION)}} AS STRING) as `GH_NATION`,
