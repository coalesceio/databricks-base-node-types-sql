...
SELECT
     ...
     CAST(TRIM(UPPER(`nation`.`nation_name`)) AS STRING) AS `nation_name` @not_null @empty @description("Nation name, trimmed and upper-cased for consistency."),
     ...
     CAST(TRIM(UPPER(`region`.`region_name`)) AS STRING) AS `region_name` @not_null @description("Region name, trimmed and upper-cased for consistency."),
     CAST(NULLIF(TRIM(`nation`.`nation_comment`), '') AS STRING) AS `nation_comment` @description("Nation comment with surrounding whitespace removed; blank comments normalized to NULL."),
